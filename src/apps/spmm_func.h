...
int task1_kernel(int tX,int tY, u_int64_t timer, u_int64_t & compute_cycles){
  ...
  u_int64_t startInd, endEdgeIndex, dense_value;
  ...
  if (global_params[3]) {//LD. Transaction is 'parked'
    ...
    dense_value = global_params[2];
    load(3); store(1); penalty = 6; // beq + 5 mem
    penalty+=task1_helper(tX,tY,global_params, startInd, endEdgeIndex, dense_value, timer+penalty);
    ...
    while(!stop){
      ...
      if (startInd<endEdgeIndex){ //EQ, JMP
        ...
        dense_value = colID;//5;//graph->dense_vector[colID]; //LD, Access to dense_vector array
...
int task2_kernel(int tX,int tY, u_int64_t timer, u_int64_t & compute_cycles){
  ...
  //int dense_value = IQ(1).dequeue().data;
  load(3);
  ...
  while (start_index < end_index){
    ...
    while(i<end_index){
      ...
      //int value_to_accum = dense_value * sparse_val;
      int value = 5 * sparse_val;
...
int task3_kernel(int tX,int tY, u_int64_t timer, u_int64_t & compute_cycles){
  ...
  //u_int32_t value_to_accum = IQ(2).dequeue().data;
  u_int32_t value = IQ(2).dequeue().data;
  load(2);

  int penalty = check_dcache(tX,tY,ret,rowID,timer, msg.time);
  ret[rowID] += value;
  store(1);
